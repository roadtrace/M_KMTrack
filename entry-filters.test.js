...
function harness(){
  ...
  for(const name of ['getLogFilters','entriesForExport','toDMM','kmToCsvNumber','xmlEscape','excelColumnName','inspectionWorkbookRows','buildInspectionWorkbook','exportTimestamp','safeBackupFilename']){
...
for(const button of ['export-btn','backup-btn']){
  test(`${button}: cancellation and empty filters do not download or read photos`,async()=>{
    ...
    control('entry-filter-day').value='2026-09-03';
    state.accept=false;
    ...
    control('entry-filter-day').value='2026-09-05';
    await control(button).click();
...
test('ZIP workbook, photos and manifest use one filtered snapshot across async work',async()=>{
  ...
  assert.ok(manifest.entries.every(entry=>!Object.hasOwn(entry,'id')));
...
test('map toggle creates plain non-interactive red dots, filters by day, refreshes and hides',()=>{
  ...
  control('map-entry-day').value='2026-09-03';
  context.updateMapEntries();
